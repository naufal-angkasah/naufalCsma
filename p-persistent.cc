...
NS_LOG_COMPONENT_DEFINE("WifiCsmaPPersistent");

void experiment(uint32_t nNodes, uint32_t packetSize, bool verbose, bool pcap, 
                uint32_t simTime, uint32_t maxPackets, uint32_t interval, uint32_t serverNode, double p)
{
  std::cout << "Running WiFi P-persistent simulation with " << nNodes << " nodes (AP=" << serverNode << ")..." << std::endl;
  ...
  if (verbose) {
    ...
    LogComponentEnable("WifiCsmaPPersistent", LOG_LEVEL_INFO);
  }
  ...
  WifiMacHelper mac;
  Ssid ssid = Ssid("wifi-persistent");
  ...
  for (uint32_t i = 0; i < nNodes; i++) {
    if (i != serverNode) {
      ...
      double delay = rand->GetValue() > p ? rand->GetValue(0.01, 1.0) : 0.0;
      app.Start(Seconds(2.0 + 0.1 * i + delay));
  ...
  for (uint32_t i = 0; i < nNodes; i++) {
    if (i != serverNode) {
      ...
      double delay = rand->GetValue() > p ? rand->GetValue(0.01, 1.0) : 0.0;
      app.Start(Seconds(3.0 + 0.1 * i + delay));
      app.Stop(Seconds(simTime));
  ...
  AnimationInterface anim("wifi-p-persistent.xml");
...
int main(int argc, char *argv[])
{
  ...
  cmd.AddValue("p", "Persistence probability", p);
  cmd.Parse(argc, argv);

  experiment(nNodes, packetSize, verbose, pcap, simTime, maxPackets, interval, serverNode, p);
