...
NS_LOG_COMPONENT_DEFINE("WifiCsmaNPersistent");
...
{
  std::cout << "Running WiFi non-persistent CSMA simulation with " << nNodes << " nodes (AP=" << serverNode << ")..." << std::endl;
  ...
  if (verbose) {
    ...
    LogComponentEnable("WifiCsmaNPersistent", LOG_LEVEL_INFO);
  }
  ...
  WifiMacHelper mac;
  Ssid ssid = Ssid("wifi-npersistent");
  ...
  for (uint32_t i = 0; i < nNodes; i++) {
    if (i != serverNode) {
      ...
      double backoff = rand->GetValue(0.01, 1.0); // Non-persistent wait
      app.Start(Seconds(2.0 + 0.1 * i + backoff));
  ...
  for (uint32_t i = 0; i < nNodes; i++) {
    if (i != serverNode) {
      ...
      double backoff = rand->GetValue(0.01, 1.0); // Non-persistent wait
      app.Start(Seconds(3.0 + 0.1 * i + backoff));
      app.Stop(Seconds(simTime));
  ...
  AnimationInterface anim("wifi-npersistent.xml");
